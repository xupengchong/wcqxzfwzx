<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="list">
      <el-table-column label="案件名称" align="center" prop="reason" />
      <!-- <el-table-column label="状态" align="center" prop="result">
        <template slot-scope="scope">
          <dict-tag :type="DICT_TYPE.BPM_PROCESS_INSTANCE_RESULT" :value="scope.row.result"/>
        </template>
      </el-table-column> -->
      <el-table-column label="当事人" align="center" prop="reason" />
      ...
      <el-table-column label="证件号码" align="center" prop="id" />
      <el-table-column label="律师名" align="center" prop="reason" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="200">
        <template slot-scope="scope">
          <!-- <el-button size="mini" type="text" icon="el-icon-delete" @click="handleCancel(scope.row)" v-hasPermi="['bpm:oa-leave:create']" v-if="scope.row.result === 1">
            取消请假
          </el-button> -->
          <el-button size="mini" type="text" icon="el-icon-view" @click="handleDetail(scope.row)"
                     v-hasPermi="['bpm:oa-leave:query']">详情</el-button>
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleProcessDetail(scope.row)">审批</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
...
export default {
  ...
  methods: {
    ...
    /** 详情按钮操作 */
    handleDetail(row) {
      this.$router.push({ path: "/bpm/ywtb/clsp/detail", query: { id: row.id}});
    },
    // 审批
    handleProcessDetail(row) {
      this.$router.push({ path: "/bpm/ywtb/clsp/approval", query: { id: row.processInstanceId}});
    },
    /** 查看审批进度的操作 */
    // handleProcessDetail(row) {
